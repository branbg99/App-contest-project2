...
def main(argv=None):
    ...
    # Optional: write list of successfully processed paper IDs for recent-only indexing
    vlist_path = os.environ.get('PROJECTSEARCHBAR_VLIST')
    def vlist_reset():
        if not vlist_path:
            return
        try:
            Path(vlist_path).parent.mkdir(parents=True, exist_ok=True)
            Path(vlist_path).write_text('', encoding='utf-8')
        except Exception:
            pass
    def vlist_append(pid: str):
        if not vlist_path:
            return
        try:
            with open(vlist_path, 'a', encoding='utf-8') as f:
                f.write(pid + "\n")
        except Exception:
            pass
