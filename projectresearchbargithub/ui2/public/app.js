(() => {
  ...
  // Tools controls
  const btnToolsDownload = document.getElementById('btnToolsDownload');
  const dlFrom = document.getElementById('optDlFrom');
  const dlUntil = document.getElementById('optDlUntil');
  const dlMax = document.getElementById('optDlMax');
  const dlSleep = document.getElementById('optDlSleep');
  const dlStatus = document.getElementById('toolsDownloadStatus');
  const btnToolsVectorize = document.getElementById('btnToolsVectorize');
  const vecLimit = document.getElementById('optVecLimit');
  const vecWorkers = document.getElementById('optVecWorkers');
  const vecStatus = document.getElementById('toolsVectorizeStatus');

  // Simple animated status helper: cycles dots (. .. ...)
  function setAnimatedStatus(el, baseText, animate) {
    try {
      if (!el) return;
      el._base = String(baseText || '');
      if (animate) {
        if (el._dotsTimer) return; // already animating
        let i = 0;
        el._dotsTimer = setInterval(() => {
          try {
            const n = (i % 3) + 1;
            const dots = ' ' + '.'.repeat(n); // " .", " ..", " ..."
            el.textContent = el._base + dots;
            i++;
          } catch (e) {}
        }, 400);
      } else {
        if (el._dotsTimer) { clearInterval(el._dotsTimer); el._dotsTimer = null; }
        el.textContent = el._base;
      }
    } catch (e) {}
  }
  ...
  async function search() {
    ...
    try {
      ...
      // Helper: normalize legacy arXiv ids like "math_0404297" -> "math/0404297" for links only
      function arxivAbsUrl(pid){
        try{
          const m = String(pid).match(/^([A-Za-z\-]+)_(\d{7})(?:v\d+)?$/);
          if (m) return `https://arxiv.org/abs/${m[1]}/${m[2]}`;
        }catch(e){}
        return `https://arxiv.org/abs/${pid}`;
      }
      function arxivPdfUrl(pid){
        try{
          const m = String(pid).match(/^([A-Za-z\-]+)_(\d{7})(?:v\d+)?$/);
          if (m) return `https://arxiv.org/pdf/${m[1]}/${m[2]}.pdf`;
        }catch(e){}
        return `https://arxiv.org/pdf/${pid}.pdf`;
      }
      ...
      j.results.forEach((rec) => {
        ...
        li.innerHTML = `
          <div class="res-head">
            <div><a class="title" href="${arxivAbsUrl(rec.paperId)}" target="_blank">${rec.paperId}</a> — <span class="muted">${rec.kind}</span></div>
          ...
          <div><a target="_blank" href="${arxivAbsUrl(rec.paperId)}">arXiv</a> · <a target="_blank" href="${arxivPdfUrl(rec.paperId)}">PDF</a></div>
        `;
        ...
        if (paperIds.length) {
          ...
          if (m && m.ok && m.meta) {
            paperIds.forEach((pid) => {
              ...
              if (headLeft) {
                const link = `<a class="title" href="${arxivAbsUrl(pid)}" target="_blank">${escapeHtml(title)}</a>`;
  ...
  async function startToolsDownload() {
    if (!btnToolsDownload) return;
    btnToolsDownload.disabled = true;
    const orig = btnToolsDownload.textContent;
    btnToolsDownload.textContent = 'Starting...';
    try {
      const payload = {
        from: dlFrom && dlFrom.value || '2010-01-01',
        until: dlUntil && dlUntil.value || null,
      ...
      const j = await r.json();
      if (!(j && j.ok && j.started)) {
        if (dlStatus) dlStatus.textContent = (j && j.reason === 'busy') ? 'Download already running' : 'Failed to start download';
        return;
      }
      const runId = j.run_id || null;
      setAnimatedStatus(dlStatus, 'Starting download', true);
      let tries = 0;
      const t = setInterval(async () => {
        tries++;
        try {
          const rs = await fetch('/api/tools/status?kind=download');
          const s = await rs.json();
          if (!s || !s.state) return;
          if (runId && s.run_id && s.run_id !== runId) return;
          if (s.state === 'downloading') {
            const d = s.downloaded||0, sk=s.skipped||0, er=s.errors||0, pg=s.pages||0, mx=s.max||0;
            setAnimatedStatus(dlStatus, `Downloading — pages ${pg}, downloaded ${d}/${mx}${sk?`, skipped ${sk}`:''}${er?`, errors ${er}`:''}`, true);
          } else if (s.state === 'done') {
            const d = s.downloaded||0, sk=s.skipped||0, er=s.errors||0;
            setAnimatedStatus(dlStatus, `Download complete — ${d} ok${sk?`, ${sk} skipped`:''}${er?`, ${er} errors`:''}`, false);
            clearInterval(t);
            btnToolsDownload.disabled = false; btnToolsDownload.textContent = orig || 'Download Papers';
          } else if (s.state === 'error') {
            setAnimatedStatus(dlStatus, `Download error: ${s.error||''}`, false);
            clearInterval(t);
            btnToolsDownload.disabled = false; btnToolsDownload.textContent = orig || 'Download Papers';
          } else if (s.state === 'starting') {
            setAnimatedStatus(dlStatus, 'Starting download', true);
          } else if (s.state === 'idle') {
            setAnimatedStatus(dlStatus, 'Idle', false);
          }
        } catch(e) { /* ignore */ }
        if (tries > 3600) { clearInterval(t); }
      }, 1000);
    } catch (e) {
      if (dlStatus) dlStatus.textContent = 'Failed to start download';
    } finally {
      // keep disabled while running; re-enabled in status handler on done/error
    }
  }

  async function startToolsVectorize() {
    if (!btnToolsVectorize) return;
    btnToolsVectorize.disabled = true;
    const orig = btnToolsVectorize.textContent;
    btnToolsVectorize.textContent = 'Starting...';
    try {
      const payload = {};
      if (vecLimit && vecLimit.value) payload.limit = parseInt(vecLimit.value, 10);
      if (vecWorkers && vecWorkers.value) payload.workers = vecWorkers.value;
      const r = await fetch('/api/tools/vectorize', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) });
      const j = await r.json();
      if (!(j && j.ok && j.started)) {
        if (vecStatus) vecStatus.textContent = (j && j.reason === 'busy') ? 'Vectorization already running' : 'Failed to start vectorization';
        btnToolsVectorize.disabled = false; btnToolsVectorize.textContent = orig || 'Vectorize Papers';
        return;
      }
      const runId = j.run_id || null;
      setAnimatedStatus(vecStatus, 'Starting vectorization', true);
      let tries = 0;
      const t = setInterval(async () => {
        tries++;
        try {
          const rs = await fetch('/api/tools/status?kind=vectorize');
          const s = await rs.json();
          if (!s || !s.state) return;
          if (runId && s.run_id && s.run_id !== runId) return;
          if (s.state === 'vectorizing') {
            const tot = s.total_files||0, dn=s.done_files||0, sk=s.skipped||0;
            const base = tot ? `Vectorizing — ${dn}/${tot} done${sk?`, skipped ${sk}`:''}` : 'Vectorizing';
            setAnimatedStatus(vecStatus, base, true);
          } else if (s.state === 'done') {
            setAnimatedStatus(vecStatus, 'Vectorization complete.', false);
            clearInterval(t);
            btnToolsVectorize.disabled = false; btnToolsVectorize.textContent = orig || 'Vectorize Papers';
          } else if (s.state === 'error') {
            setAnimatedStatus(vecStatus, `Vectorization error: ${s.error||''}`, false);
            clearInterval(t);
            btnToolsVectorize.disabled = false; btnToolsVectorize.textContent = orig || 'Vectorize Papers';
          } else if (s.state === 'starting') {
            setAnimatedStatus(vecStatus, 'Starting vectorization', true);
          } else if (s.state === 'idle') {
            setAnimatedStatus(vecStatus, 'Idle', false);
          }
        } catch(e) { /* ignore */ }
        if (tries > 3600) { clearInterval(t); }
      }, 1000);
    } catch (e) {
      if (vecStatus) vecStatus.textContent = 'Failed to start vectorization';
      btnToolsVectorize.disabled = false; btnToolsVectorize.textContent = orig || 'Vectorize Papers';
    }
  }
  ...
  // Bind tool buttons (if present)
  if (btnToolsDownload && !btnToolsDownload._psbBound) { btnToolsDownload._psbBound = true; btnToolsDownload.addEventListener('click', startToolsDownload); }
  if (btnToolsVectorize && !btnToolsVectorize._psbBound) { btnToolsVectorize._psbBound = true; btnToolsVectorize.addEventListener('click', startToolsVectorize); }
