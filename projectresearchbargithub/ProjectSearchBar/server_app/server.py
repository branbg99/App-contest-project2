...
class SearchRequestHandler(SimpleHTTPRequestHandler):
    ...
    def _offline_fallback_meta(self, arxid: str) -> dict | None:
        # Attempt offline fallback using local LaTeX first, then vectors
        # 1) Try to parse basic fields from local LaTeX sources (title, authors, abstract)
        try:
            raw = self._read_raw_latex(arxid)
            s = (raw or '').strip()
            if s:
                # Extract \title{...}
                m_title = re.search(r"\\title\s*\{([^}]*)\}", s, re.IGNORECASE | re.DOTALL)
                title = (m_title.group(1).strip() if m_title else arxid)
                # Extract \author{...}
                m_auth = re.search(r"\\author\s*\{([^}]*)\}", s, re.IGNORECASE | re.DOTALL)
                authors_raw = (m_auth.group(1).strip() if m_auth else '')
                # Extract abstract from environment
                m_abs = re.search(r"\\begin\{abstract\}([\s\S]*?)\\end\{abstract\}", s, re.IGNORECASE)
                abstract_raw = (m_abs.group(1).strip() if m_abs else '')
                def _strip_tex(a: str) -> str:
                    # Remove common LaTeX commands and braces for a readable preview
                    a = re.sub(r"\\[a-zA-Z@]+\*?\s*(\[[^\]]*\])?\s*(\{[^}]*\})?", lambda m: (m.group(2)[1:-1] if m.group(2) else ' '), a)
                    a = a.replace('{', ' ').replace('}', ' ')
                    a = re.sub(r"\s+", " ", a)
                    return a.strip()
                title_s = _strip_tex(title) if title else arxid
                authors_s = _strip_tex(authors_raw)
                # split authors on common separators ; and , and 'and'
                authors_list: list[str] = []
                if authors_s:
                    tmp = re.split(r";|,|\band\b", authors_s)
                    authors_list = [t.strip() for t in tmp if t and t.strip()]
                abstract_s = _strip_tex(abstract_raw)[:1200] if abstract_raw else ''
                if title_s or abstract_s or authors_list:
                    return {
                        'arxiv_id': arxid,
                        'title': title_s or arxid,
                        'authors': authors_list,
                        'subjects': [],
                        'abstract': abstract_s,
                        'doi': None,
                        'source': 'offline_latex',
                        'ts': 0,
                    }
        except Exception:
            pass
        # 2) Fallback to local vectors (first paragraph chunk)
        import json as _json
