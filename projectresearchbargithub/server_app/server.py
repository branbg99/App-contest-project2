...
class SearchRequestHandler(SimpleHTTPRequestHandler):
    ...
    # Tool runners (download/vectorize-only)
    _dl_active = False
    _dl_lock = threading.Lock()
    _dl_id: str | None = None
    _vec_active = False
    _vec_lock = threading.Lock()
    _vec_id: str | None = None
    def _normalize_model(self, m: str | None) -> str:
        """Normalize model IDs to canonical, pass-through supported values.

        - Accepts: gpt-5, gpt-5-mini, gpt-4o, gpt-4o-mini
        - Maps historical aliases like chatgpt-4o* and chatgpt-5* to the above
        - Defaults to gpt-4o-mini when empty/invalid
        """
        try:
            s = (m or '').strip().lower()
            if not s:
            ...
            if s == 'chatgpt-4o':
                return 'gpt-4o'
            if s == 'chatgpt-4o-mini':
                return 'gpt-4o-mini'
            if s == 'chatgpt-5':
                return 'gpt-5'
            if s == 'chatgpt-5-mini':
                return 'gpt-5-mini'
            # Known supported ids (pass-through)
            if s in ('gpt-5', 'gpt-5-mini', 'gpt-4o', 'gpt-4o-mini'):
                return s
            # Unknown string: pass through as-is to allow future models
            return s
        except Exception:
            return 'gpt-4o-mini'
    ...
    def do_GET(self):
        try:
            ...
            if self.path.startswith('/api/tools/status'):
                return self.api_tools_status()
            ...
            if self.path.startswith('/api/agent/settings'):
                return self.api_agent_settings_get()
            return super().do_GET()
    ...
    def do_POST(self):
        try:
            ...
            if self.path.startswith('/api/agent/settings'):
                return self.api_agent_settings_post()
            if self.path.startswith('/api/tools/download'):
                return self.api_tools_download()
            if self.path.startswith('/api/tools/vectorize'):
                return self.api_tools_vectorize()
    ...
    def api_paper_meta(self):
        try:
            ...
            # Normalize helper: map legacy ids like "math_0404297" -> "math/0404297" (strip version suffix)
            def _normalize_arxiv_id(s: str) -> str:
                try:
                    s2 = s.strip()
                    # Strip simple version suffix
                    if 'v' in s2 and s2.split('v')[-1].isdigit():
                        s2 = s2.split('v')[0]
                    # Legacy category ids encoded with underscore in local files
                    # e.g., "math_0404297" -> "math/0404297"; allow hyphenated cats like "cond-mat"
                    import re as _re
                    m = _re.match(r'^([A-Za-z\-]+)_(\d{7})$', s2)
                    if m:
            ...
            if missing:
                try:
                    # Fetch using normalized ids, but map results back to original request ids
                    fetched: dict[str, dict] = {}
                    self._fetch_and_cache_arxiv_meta([_normalize_arxiv_id(x) for x in missing], cache_path, fetched)
                    # Map normalized keys to each original pid
                    for orig in list(missing):
                        norm = _normalize_arxiv_id(orig)
                        m = fetched.get(norm)
                        if m:
                            try:
                                # Cache under the original pid key for future quick hits
                                cp = cache_path(orig)
                                cp.write_text(_json.dumps(m, ensure_ascii=False), encoding='utf-8')
                            except Exception:
                                pass
                            meta[orig] = m
                except Exception:
    ...
    def _offline_fallback_meta(self, arxid: str) -> dict | None:
        ...
        for root in (v2, v1):
            try:
                ...
                if cj.exists():
                    # Prefer first paragraph; otherwise fall back to the first chunk with any text
                    ...
                    with cj.open('r', encoding='utf-8', errors='ignore') as f:
                        for line in f:
                            try:
                                ...
                                if not first_any and txt:
                                    first_any = txt
                                if o.get('kind') == 'paragraph' and txt:
                                    first_paragraph = txt
                                    break
                            except Exception:
                                continue
                    chosen = first_paragraph or first_any
                    if chosen:
                        return {
                            'arxiv_id': arxid,
                            'title': arxid,
    ...
    # --- Tools API: status + runners ---
    def _tools_status_path(self, kind: str) -> Path:
        name = 'tools_download_status.json' if kind == 'download' else 'tools_vectorize_status.json'
        return config.DATA_DIR / name

    def api_tools_status(self):
        try:
            from urllib.parse import urlparse, parse_qs
            parsed = urlparse(self.path)
            qs = parse_qs(parsed.query or '')
            kind = (qs.get('kind') or [''])[0].strip().lower()
            if kind not in ('download', 'vectorize'):
                return self._write_json({'ok': False, 'error': 'invalid kind'})
            sp = self._tools_status_path(kind)
            if not sp.exists():
                return self._write_json({'ok': True, 'state': 'idle', 'ts': time.time()})
            try:
            ...
            return self._write_json(obj, 200)
        except Exception as e:
            return self._write_json({'ok': False, 'error': str(e)}, 200)

    def api_tools_download(self):
        body = self._read_json()
        out_dir = body.get('out') or None
        max_n = body.get('max')
        ...
        try:
            ...
            sleep_s = 3.0
        try:
            max_n = int(max_n) if max_n is not None else 50000
        except Exception:
            max_n = 50000
        try:
            out_path = Path(out_dir).expanduser().resolve() if out_dir else (config.DATA_DIR / 'arxiv_tex_selected')
        except Exception:
            out_path = config.DATA_DIR / 'arxiv_tex_selected'

        status_path = self._tools_status_path('download')
        tmp_status_path = status_path.with_suffix('.tmp')
        def write_status(obj: dict):
            try:
                payload = {**obj, 'ts': time.time()}
                if SearchRequestHandler._dl_id:
                    payload['run_id'] = SearchRequestHandler._dl_id
        ...
        with SearchRequestHandler._dl_lock:
            if SearchRequestHandler._dl_active:
                return self._write_json({'ok': False, 'started': False, 'reason': 'busy'})
            try:
                import uuid
                SearchRequestHandler._dl_id = uuid.uuid4().hex
            except Exception:
                SearchRequestHandler._dl_id = str(int(time.time() * 1000))
            SearchRequestHandler._dl_active = True
            try:
                status_path.unlink(missing_ok=True)  # type: ignore[arg-type]
            except Exception:
                pass
            write_status({'ok': True, 'state': 'starting', 'out': str(out_path)})
        ...
        def worker():
            try:
                ...
                errors = 0
                token = None
                write_status({'ok': True, 'state': 'downloading', 'downloaded': 0, 'skipped': 0, 'errors': 0, 'pages': 0, 'max': max_n})
                while total < max_n:
                    try:
                        ids, token = dl.oai_listrecords_math(resumption_token=token, from_date=from_date, until_date=until_date)
                    except Exception as e:
                        errors += 1
                        write_status({'ok': False, 'state': 'error', 'error': f'oai: {e}', 'downloaded': total, 'skipped': skipped, 'errors': errors, 'pages': pages})
                        break
                    pages += 1
                    if not ids:
                        break
                    for bid in ids:
                        if total >= max_n:
                            break
                        try:
                            res = dl.download_tex_tarball(bid, out_path)
                            if res == 'ok':
                                total += 1
                            ...
                            errors += 1
                        if (total + skipped + errors) % 10 == 0:
                            write_status({'ok': True, 'state': 'downloading', 'downloaded': total, 'skipped': skipped, 'errors': errors, 'pages': pages, 'last_id': bid})
                        time.sleep(sleep_s)
                    if not token:
                        break
                    time.sleep(sleep_s)
                write_status({'ok': True, 'state': 'done', 'downloaded': total, 'skipped': skipped, 'errors': errors, 'pages': pages})
            except Exception as e:
    ...
    def api_tools_vectorize(self):
        body = self._read_json()
        src = body.get('src') or None
        out = body.get('out') or None
        limit = body.get('limit')
        workers = body.get('workers') or 'auto'
        try:
            limit = int(limit) if limit is not None else None
        except Exception:
            limit = None
        try:
            src_path = Path(src).expanduser().resolve() if src else config.PAPERS_SRC
        except Exception:
            src_path = config.PAPERS_SRC
        try:
            out_path = Path(out).expanduser().resolve() if out else config.VECTORS_DIR
        except Exception:
            out_path = config.VECTORS_DIR

        status_path = self._tools_status_path('vectorize')
        tmp_status_path = status_path.with_suffix('.tmp')
        def write_status(obj: dict):
            try:
                payload = {**obj, 'ts': time.time()}
                if SearchRequestHandler._vec_id:
                    payload['run_id'] = SearchRequestHandler._vec_id
                tmp_status_path.write_text(json.dumps(payload), encoding='utf-8')
                try:
                    os.replace(str(tmp_status_path), str(status_path))
                except Exception:
        ...
        with SearchRequestHandler._vec_lock:
            if SearchRequestHandler._vec_active:
                return self._write_json({'ok': False, 'started': False, 'reason': 'busy'})
            try:
                import uuid
                SearchRequestHandler._vec_id = uuid.uuid4().hex
            except Exception:
                SearchRequestHandler._vec_id = str(int(time.time() * 1000))
            SearchRequestHandler._vec_active = True
            try:
                status_path.unlink(missing_ok=True)  # type: ignore[arg-type]
            except Exception:
                pass
            write_status({'ok': True, 'state': 'starting', 'src': str(src_path), 'out': str(out_path)})
        ...
        def worker():
            try:
                ...
                os.environ['PROJECTSEARCHBAR_BUILD_ID'] = SearchRequestHandler._vec_id or ''
                # Prepare recent-only list file for this vectorize run
                try:
                    recent_list = config.DATA_DIR / 'tools_vectorize.list'
                    try:
                        recent_list.unlink(missing_ok=True)  # type: ignore[arg-type]
                    except Exception:
                        pass
                    os.environ['PROJECTSEARCHBAR_VLIST'] = str(recent_list)
                except Exception:
                    pass
                args = ['--src', str(src_path), '--out', str(out_path)]
                if limit:
                    args += ['--limit', str(limit)]
                if workers:
                    args += ['--workers', str(workers)]
                write_status({'ok': True, 'state': 'vectorizing'})
                vec_mod.main(args)  # type: ignore[arg-type]
                ...
                try:
                    ...
                    os.environ.pop('PROJECTSEARCHBAR_BUILD_ID', None)
                    os.environ.pop('PROJECTSEARCHBAR_VLIST', None)
                except Exception:
                    pass
                with SearchRequestHandler._vec_lock:
                    SearchRequestHandler._vec_active = False
                    SearchRequestHandler._vec_id = None

        threading.Thread(target=worker, daemon=True).start()
        return self._write_json({'ok': True, 'started': True, 'src': str(src_path), 'out': str(out_path), 'run_id': SearchRequestHandler._vec_id}, 200)
        ...
        def worker():
            ...
            try:
                ...
                if build_mode == 'sharded':
                    ...
                    # Append only new papers to the existing DB and recompute norms
                    idx_args = [
                        '--scan', str(config.VECTORS_DIR),
                    ...
                    print('[index_build] Incremental merge into existing DB:', config.DB_PATH)
                    ...
                    write_status({'ok': True, 'state': 'done'})
                elif build_mode == 'recent':
                    # Build index only from the most recent vectorize run (allowlist)
                    allow = config.DATA_DIR / 'tools_vectorize.list'
                    try:
                        if (not allow.exists()) or (allow.stat().st_size == 0):
                            write_status({'ok': False, 'state': 'error', 'error': 'recent_empty', 'detail': 'No recent vectorize list found; run Vectorize first.'})
                            return
                    except Exception:
                        write_status({'ok': False, 'state': 'error', 'error': 'recent_empty'})
                        return
                    ...
                    if limit:
                        idx_args += ['--limit-papers', str(limit)]
                    write_status({'ok': True, 'state': 'indexing'})
                    print('[index_build] Recent-only merge from allowlist to', build_db)
                    os.environ['PROJECTSEARCHBAR_STATUS'] = str(status_path)
                    if SearchRequestHandler._build_id:
                        os.environ['PROJECTSEARCHBAR_BUILD_ID'] = SearchRequestHandler._build_id
                    # Apply optional tuning from settings
    ...
    def api_llm_settings_get(self):
        ...
        model = self._normalize_model(s.get('model') or 'gpt-4o-mini')
        # Normalize legacy aliases
        if isinstance(model, str):
            alias = model.strip().lower()
            if alias == 'chatgpt-4o':
                model = 'gpt-4o'
            elif alias == 'chatgpt-4o-mini':
                model = 'gpt-4o-mini'
    ...
    def api_llm_settings_post(self):
        ...
        model = self._normalize_model(body.get('model') or s.get('model') or 'gpt-4o-mini')
        # Normalize aliases
        if isinstance(model, str):
            m = model.strip().lower()
            if m == 'chatgpt-4o':
                model = 'gpt-4o'
            elif m == 'chatgpt-4o-mini':
                model = 'gpt-4o-mini'
    ...
    # -------- Agent settings (persona + defaults) --------
    def _agent_settings_path(self) -> Path:
        return config.DATA_DIR / 'agent_settings.json'
    ...
    def _read_agent_settings(self) -> dict:
        ...
        return {
            'persona': (
                'You are a single research agent. Read ONLY the attached paper and answer the user\'s instruction. '
                'If you find relevant evidence, reply with FOUND and provide exact quotes with section/theorem numbers and cite as [paperId]. '
                'If not, reply with NOT_FOUND and a brief note. Use LaTeX $...$ for math.'
            ),
            'default_top_n': 10,
            'token_budget': 64000,
            'concurrency': 1,
    ...
    def _write_agent_settings(self, obj: dict) -> None:
        p = self._agent_settings_path()
        try:
    ...
    def api_agent_settings_get(self):
        try:
            s = self._read_agent_settings()
            return self._write_json({'ok': True, 'settings': s})
        except Exception as e:
            return self._write_json({'ok': False, 'error': str(e)})

    def api_agent_settings_post(self):
        body = self._read_json()
        try:
            cur = self._read_agent_settings()
            out = dict(cur)
            s = body.get('settings') or {}
            if not isinstance(s, dict):
                return self._write_json({'ok': False, 'error': 'invalid_settings'})
            # Shallow update on known keys
            for k in ('persona','default_top_n','token_budget','concurrency','latex_only','early_stop_k'):
                if k in s:
                    out[k] = s[k]
            self._write_agent_settings(out)
            return self._write_json({'ok': True})
        except Exception as e:
            return self._write_json({'ok': False, 'error': str(e)})
